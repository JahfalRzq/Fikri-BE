...
export class UpdateSchema1760923124719 implements MigrationInterface {
    name = 'UpdateSchema1760923124719'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`certificate\` ADD \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)`);
        await queryRunner.query(`ALTER TABLE \`certificate\` ADD \`deletedAt\` datetime(6) NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`training_participant_category\` ADD CONSTRAINT \`FK_2aaf52cf931474f3e396fc20a05\` FOREIGN KEY (\`categoryId\`) REFERENCES \`category\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`certificate\` DROP COLUMN \`deletedAt\``);
        await queryRunner.query(`ALTER TABLE \`certificate\` DROP COLUMN \`updatedAt\``);
    }
