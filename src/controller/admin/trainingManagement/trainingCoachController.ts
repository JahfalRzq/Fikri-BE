...
export const getAllTrainingCoaches = async (req: Request, res: Response) => {
  try {
    const { coachName, limit: queryLimit, page } = req.query
...
export const createTrainingCoach = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const newCoach = new trainingCoach()
    ...
    return res
      ...
      .send(
        successResponse(
          "Training Coach created successfully",
          { newCoach },
          201,
        ),
      )
...
export const deleteTrainingCoach = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!coach) {
      return res
        .status(404)
        .send(validationResponse("Training Coach not found"))
    }
    ...
    return res
      .status(201)
      .send(
        successResponse("Category deleted successfully", { data: coach }, 201),
      )
  } catch (error) {
    return res.status(500).json({
      message:
