...
export const createCategoryTraining = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (trainingCode) {
      const existingCategory = await categoryTrainingRepository.findOneBy({
        trainingCode,
      })
      if (existingCategory) {
        return res
          .status(400)
          .send(validationResponse("trainingCode already used"))
      }
    }
    ...
    return res
      ...
      .send(
        successResponse(
          "Category Training created successfully",
          { data: newCategory },
          201,
        ),
      )
...
export const updateCategoryTraining = async (req: Request, res: Response) => {
  try {
    ...
    if (body.trainingCode !== undefined)
      category.trainingCode = body.trainingCode
    ...
    if (body.trainingCode) {
      ...
      if (existingCategory && existingCategory.id !== category.id) {
        return res
          .status(400)
          .send(
            validationResponse("trainingCode already used by another category"),
          )
      }
    }
...
export const deleteCategoryTraining = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!category) {
      return res
        .status(404)
        .send(validationResponse("Category Training not found"))
    }
