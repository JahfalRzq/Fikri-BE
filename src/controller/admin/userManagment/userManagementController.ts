...
export const getAllUsers = async (req: Request, res: Response) => {
  try {
    const { username, email, role, phone, limit: queryLimit, page } = req.query
...
export const getUserById = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .send(successResponse("Get User by ID Success", { data: result }, 200))
  } catch (error) {
    return res.status(500).json({
      message:
        error instanceof Error ? error.message : "Unknown error occurred",
...
export const createUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(201)
      .send(
        successResponse("User created successfully", { data: newUser }, 201),
      )
  } catch (error) {
    return res.status(500).json({
      message:
...
export const updateUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send(
        successResponse("Update Training Success", { data: updateUser }, 200),
      )
  } catch (error) {
    return res.status(500).json({
      message:
...
export const deleteUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(201)
      .send(successResponse("User deleted successfully", { data: user }, 201))
  } catch (error) {
    return res.status(500).json({
      message:
        error instanceof Error ? error.message : "Unknown error occurred",
