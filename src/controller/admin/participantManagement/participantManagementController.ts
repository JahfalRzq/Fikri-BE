...
import {
  statusTraining,
  trainingParticipant,
} from "@/model/training-participant"
...
const trainingParticipantStory =
  AppDataSource.getRepository(trainingParticipant)
    ...
    const safeParticipant = {
      ...
      training: result.trainingParticipant.training
        ? {
            ...
            trainingCode:
              result.trainingParticipant.training.categoryTraining.trainingCode,
            startDateTraining:
              result.trainingParticipant.training.startDateTraining,
            endDateTraining:
              result.trainingParticipant.training.endDateTraining,
          }
        : null,
...
export const createParticipant = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // cek training
    const trainingRecord = await trainingRepository.findOneBy({
      id: body.training,
    })
    if (!trainingRecord) {
    ...
    return res.status(201).send(
      successResponse(
        "New participant created and added to training",
        {
          ...
          participantTraining: newParticipantTraining,
        },
        201,
      ),
    )
