...
export const participantSeeder = async (req: Request, res: Response) => {
  try {
    ...
    if (trainings.length === 0) {
      return res
        .status(404)
        .send(errorResponse("No training found, seed training first", 404))
    }
    ...
    for (const data of participantsSeed) {
      ...
      if (!trainingType) {
        console.warn(
          `Training ${data.training} not found, skipping participant ${data.email}`,
        )
        continue
      }
    ...
    return res
      .status(201)
      .send(successResponse("Participants seeded successfully", null, 201))
  } catch (error) {
    return res
      .status(500)
      .send(
