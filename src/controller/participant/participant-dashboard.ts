...
export const getCertificatesByLoggedInParticipant = async (req: Request, res: Response) => {
  try {
    // Ambil user id dari jwt payload
    ...
    // Cari participant yang terkait dengan user ini
    ...
    const participantId = userParticipant.id; // <-- digunakan untuk filter

    // Build query: dari certificate -> trainingParticipant -> participant -> training -> trainingCategory -> category, juga join coach
    ...
    // Optional filter: search against trainingName or certificate noLiscense
    if (search) {
      queryBuilder.andWhere(
        `(training.trainingName LIKE :search OR certificate.noLiscense LIKE :search)`,
        { search: `%${search}%` }
      );
    }
    ...
    // Map hasil agar frontend dapat langsung konsumsi
    const certificateData = certificates.map((cert) => {
      const tp = (cert as any).trainingParticipant;
      const participant = tp?.participant;
      const training = tp?.training;
      const coach = tp?.coach;
      const trainingCategoryRel = (training && (training as any).trainingCategory) || [];

      // ambil nama category dari relasi trainingCategory -> category
      const categoryName =
        Array.isArray(trainingCategoryRel) && trainingCategoryRel.length > 0
          ? trainingCategoryRel[0]?.category?.categoryName ?? null
          : null;
      ...
      return {
        ...
        trainingParticipant: {
          ...
          training: {
            ...
            // fields tambahan untuk UI:
            category: categoryName,
            price: training?.price ?? null,
            startDateTraining: training?.startDateTraining ?? null,
            endDateTraining: training?.endDateTraining ?? null,
          },
          coach: {
            id: coach?.id ?? null,
            name: coach?.coachName ?? null,
          },
        },
      };
    });
    ...
    console.error("Error in getCertificatesByLoggedInParticipant:", error);
